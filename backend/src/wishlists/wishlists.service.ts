...
export class WishlistsService {
  ...
  async findAllWishes(): Promise<Wishlist[]> {
  ...
  async createWishes(
    ...
    for (const itemId of createWishlistDto.itemsId) {
      items.push(await this.wishesService.findOneWish(itemId));
    }
  ...
  async findOneWishes(id: number): Promise<Wishlist> {
  ...
  async removeWishes(id: number, user: User): Promise<Wishlist> {
    const wishlist = await this.wishlistsRepository.findOne({
      where: { id },
